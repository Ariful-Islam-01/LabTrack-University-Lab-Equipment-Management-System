                ...
                <div class="card-body p-4">
                    <form action="{{ route('equipment.update', $equipment->equipment_id) }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" 
                                   class="form-control bg-light @error('equipment_id') is-invalid @enderror" 
                                   id="equipment_id" 
                                   name="equipment_id" 
                            ...
                            <select class="form-select @error('category_id') is-invalid @enderror" 
                                ...
                                @foreach ($categories as $category)
                                    <option value="{{ $category->category_id }}" 
                                            {{ old('category_id', $equipment->category_id) == $category->category_id ? 'selected' : '' }}>
                            ...
                            <select class="form-select @error('lab_id') is-invalid @enderror" 
                                ...
                                @foreach ($labs as $lab)
                                    <option value="{{ $lab->lab_id }}" 
                                            {{ old('lab_id', $equipment->lab_id) == $lab->lab_id ? 'selected' : '' }}>
                                        {{ $lab->lab_name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="number" 
                                   ...
                                   id="quantity" 
                                   name="quantity" 
                                   value="{{ old('quantity', $equipment->total_quantity) }}">
                            @error('quantity')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
